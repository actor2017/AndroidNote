...
public class MessageInfoUtil {
    ...
    public static MessageInfo buildImageMessage(final Uri uri, boolean compressed, boolean appPohto) {
        ...
        if (!appPohto) {
            ...
            info.setDataUri(TencentFiles.getUriFromPath(copInfo.getPath()));
        } else {
            info.setDataUri(uri);
            int size[] = ImageUtil.getImageSize(uri);
            String path = TencentFiles.getPathFromUri(uri);
            ele.setPath(path);
    ...
    public static MessageInfo buildFileMessage(Uri fileUri) {
        String filePath = TencentFiles.getPathFromUri(fileUri);
    ...
    public static List<MessageInfo> TIMMessages2MessageInfos(List<TIMMessage> timMessages, boolean isGroup) {
        if (timMessages == null) return null;
        ...
        for (int i = 0; i < timMessages.size(); i++) {
            ...
            if (info != null) messageInfos.add(info);
        }
        return messageInfos;
    }
    ...
    public static MessageInfo TIMMessage2MessageInfo(TIMMessage timMessage, boolean isGroup) {
        if (timMessage == null || timMessage.status() == TIMMessageStatus.HasDeleted)//=null或已经删除
            return null;
        ...
        if (isGroup) {
            ...
            } else msgInfo.setFromUser(sender);
        } else {
...
//                            }
//                        }
//                    } else user = groupTips.getOpUserInfo().getIdentifier();
//                    String message = user;
...
//                    break;
//                case Face:
//                    TIMFaceElem faceEle = (TIMFaceElem) ele;
//                    if (faceEle.getIndex() < 1 || faceEle.getData() == null) {
...
//                        msgInfo.setDataUri(TencentFiles.getUriFromPath(videoEle.getVideoPath()));
//                    } else {
...
//            }
            ...
            if (type == TIMElemType.Custom) {
                ...
                if (data.equals(GROUP_CREATE)) {
                    ...
                    msgInfo.setExtra("[自定义消息]");
                    //FIXME HIHI交友这个项目屏蔽自定义消息, 否则C2C语音聊天后, 消息聊天列表也有"[自定义消息]"
                    //如果不屏蔽, 下面一行注释掉
                    return null;
                }
                ...
                if (type == TIMElemType.Text) {
                    ...
                    //https://cloud.tencent.com/document/product/269/9236
                    TIMGroupSystemElem systemElem = (TIMGroupSystemElem) ele;
                    byte[] userData = systemElem.getUserData();//发送的系统消息...
                    String s = null;
                    if (userData != null) {
                        try {
                            s = new String(userData, "UTF-8");
                        } catch (UnsupportedEncodingException e) {
                            e.printStackTrace();
                        }
                    }
                    msgInfo.setExtra(s);
                } else QLog.e("消息类型type=", String.valueOf(type.value()));
