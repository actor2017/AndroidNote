...
//                switchCollapsingToolbarLayoutScrollable(ctlCollapsingToolbarLayout);
                nsvNestedScrollView.setNestedScrollingEnabled(false);
                break;
            case R.id.fab:
                //如果正在播放视屏
                if (ivHeader.getVisibility() == View.INVISIBLE || ivHeader.getVisibility() ==
                        View.GONE) {
//                    switchCollapsingToolbarLayoutScrollable(ctlCollapsingToolbarLayout);
                    nsvNestedScrollView.setNestedScrollingEnabled(true);
                }
                break;
        }
    }
    ...
    private void switchCollapsingToolbarLayoutScrollable(CollapsingToolbarLayout collapsingToolbarLayout){
        ...
        if (scrollFlags == AppBarLayout.LayoutParams.SCROLL_FLAG_SCROLL) {//初始化
        ...
        if (layoutParams.getScrollFlags() != scrollFlags) {//恢复滑动状态
            ...
            layoutParams.setScrollFlags(AppBarLayout.LayoutParams.SCROLL_FLAG_SCROLL);//清空滑动状态
        }

        System.out.println("layoutParams.getScrollFlags():"+layoutParams.getScrollFlags());
    }
