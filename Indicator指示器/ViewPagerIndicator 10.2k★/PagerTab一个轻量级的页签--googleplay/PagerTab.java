package com.kuchuan.education.widget;
...
import com.kuchuan.education.activity.BaseActivity;
import com.kuchuan.education.utils.UiUtils;

/**
 * 本类来源googleplayer?,自定义轻量级ViewPager的Indicator(页签)
 * 0.activity extends baseActivity
 * 1.setViewPager();ViewPager设置Adapter后,绑定ViewPager
 * 2.setOnPageChangeListener:设置监听，因为Tab会监听ViewPager的状态，所以不要给ViewPager设置监听了，设置给Tab，由Tab转发
 * 3.onInterceptTouchEvent:触摸事件是否拦截的方法
 * 4.scrollSelf:滚动到指定的child
 * 5.IconTabProvider:如果指示器希望是图片，则继承该接口
 */
public class PagerTab extends ViewGroup {
	...
	private int mTabTextColorResId = R.drawable.tab_text_color; //tab文字颜色
	...
	private void onViewPagerChanged() {
		...
		for (int i = 0; i < mTabCount; i++) {
			if (mViewPager.getAdapter() instanceof IconTabProvider) {//如果想要使用icon作为tab，则需要adapter实现IconTabProvider接口
				...
				if(title == null){
					Toast.makeText(getContext(), "请在ViewPager的Adapter里面重写getPageTitle方法", Toast.LENGTH_SHORT).show();
					title = "?";
				}
			...
			viewTreeObserver.addOnGlobalLayoutListener(new ViewTreeObserver.OnGlobalLayoutListener() {
				...
				public void onGlobalLayout() {
					...
					if (mDelegatePageListener != null) {
						...
						mPageListener.onPageSelected(mCurrentPosition);
					}
				}
			});
		}
