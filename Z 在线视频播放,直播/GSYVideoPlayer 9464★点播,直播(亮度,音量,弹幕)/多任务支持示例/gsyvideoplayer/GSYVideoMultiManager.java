...
public class GSYVideoMultiManager extends GSYVideoBaseManager {

    public static final String TAG = "GSYVideoMultiManager";
    public static final int SMALL_ID = R.id.custom_small_id;
    public static final int FULLSCREEN_ID = R.id.custom_full_id;
    private static Map<String, GSYVideoMultiManager> sMap = new HashMap<>();

    public GSYVideoMultiManager() {
        init();
    }
     ...
     * 单例管理器
     */
    public static synchronized Map<String, GSYVideoMultiManager> instance() {
        return sMap;
    }
     ...
     * 单例管理器
     */
    public static synchronized GSYVideoMultiManager getCustomManager(String key) {
        if (TextUtils.isEmpty(key)) {
            throw new IllegalStateException("key not be empty");
        }
        GSYVideoMultiManager customManager = sMap.get(key);
        if (customManager == null) {
            customManager = new GSYVideoMultiManager();
    ...
    public static void onPauseAll() {
        if (sMap.size() > 0) {
            for (Map.Entry<String, GSYVideoMultiManager> header : sMap.entrySet()) {
    ...
    public static void clearAllVideo() {
        if (sMap.size() > 0) {
            for (Map.Entry<String, GSYVideoMultiManager> header : sMap.entrySet()) {
                GSYVideoMultiManager.releaseAllVideos(header.getKey());
            }
        }
        sMap.clear();
    }
